...
export default function KarakamshaPanel({ birthData }: { birthData: BirthData | null }) {
  ...
  if (!birthData) return <div style={{ padding: 20, color: 'var(--text3)', fontSize: 13 }}>{t('Enter birth data to compute Karakamsha.')}</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#f59e0b' }}>{t('Karakamsha Chart')}</div>
          <div style={{ fontSize: 11, color: 'var(--text3)' }}>{t('Atmakaraka in Navamsa · Jaimini')}</div>
        </div>
      ...
      {data && (
        <>
          ...
          <div style={{ ...card, display: 'flex', flexWrap: 'wrap', gap: 24 }}>
            <div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 4 }}>{t('Atmakaraka')}</div>
              <div style={{ fontSize: 24, fontWeight: 800, color: PLANET_COLORS[data.atmakaraka] }}>{data.atmakaraka}</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 4 }}>{t('Karakamsha Lagna')}</div>
              <div style={{ fontSize: 24, fontWeight: 800, color: '#f59e0b' }}>{data.karakamsha_sign}</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 4 }}>{t('In D1 House')}</div>
            ...
            {data.swamsha && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: '#f59e0b18', border: '1px solid #f59e0b44', borderRadius: 10 }}>
                <span style={{ fontWeight: 600, color: '#f59e0b' }}>{t('Swamsha')}</span>
                <span style={{ fontSize: 11, color: 'var(--text3)' }}>{t('KL = D1 Lagna')}</span>
              </div>
            )}
            {data.planets_in_kl.length > 0 && (
              <div>
                <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 4 }}>{t('Planets in KL')}</div>
          ...
          <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid var(--border)', paddingBottom: 0 }}>
            {(['overview', 'houses', 'yogas', 'karakas'] as const).map(tb => (
              <button key={tb} onClick={() => setTab(tb)} style={{
                ...
                {tb === 'karakas' ? t('Chara Karakas') : t(tb.charAt(0).toUpperCase() + tb.slice(1))}
              </button>
            ))}
          </div>
          ...
          {tab === 'overview' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {data.ak_sign_interpretation && (
                <div style={card}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#f59e0b', marginBottom: 12 }}>{t('Soul Purpose — AK in')} {data.karakamsha_sign}</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    {[
                      [t('Core Purpose'), data.ak_sign_interpretation.soul_purpose],
                      [t('Career Dharma'), data.ak_sign_interpretation.career],
                      [t('Spiritual Path'), data.ak_sign_interpretation.spirituality],
                      [t('Soul Traits'), data.ak_sign_interpretation.traits],
                    ].map(([label, val]) => (
              ...
              {data.planets_in_kl.length > 0 && (
                <div style={card}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#f59e0b', marginBottom: 12 }}>{t('Planets in Karakamsha Lagna')}</div>
              ...
              <div style={card}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#f59e0b', marginBottom: 12 }}>{t('D9 Navamsa Planets')}</div>
          ...
          {tab === 'yogas' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {data.karakamsha_yogas.length === 0 && (
                <div style={{ padding: 16, color: 'var(--text3)', fontSize: 13 }}>{t('No special Karakamsha yogas detected.')}</div>
              )}
              {data.karakamsha_yogas.map((y, i) => (
                <div key={i} style={{ ...card }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text4)' }}>{t('Source')}: {y.source}</div>
                </div>
              ))}
            </div>
          )}
                ...
                <thead>
                  <tr style={{ background: 'var(--surface2)' }}>
                    {['Planet', 'Karaka', 'Degree', 'D1 Sign', 'D9 Sign'].map(h => (
                      <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 11, color: 'var(--text3)', fontWeight: 600, borderBottom: '1px solid var(--border)' }}>{t(h)}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {data.karakas.map((k, i) => (
                    <tr key={k.planet} style={{ borderBottom: '1px solid var(--border)', background: i === 0 ? '#f59e0b10' : 'transparent' }}>
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        ...
                        {i === 0 && <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--text4)' }}>← {t('Atmakaraka')}</span>}
                      </td>
