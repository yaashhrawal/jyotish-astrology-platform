...
export default function BhavaChaliPanel({ birthData }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {shifted.length > 0 && (
        <div style={{ background: 'var(--red-bg)', border: '1px solid var(--red)', borderRadius: 12, padding: '14px 18px' }}>
          ...
          {shifted.map((p: any) => (
            <div key={p.planet} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 0', borderBottom: '1px solid var(--red)' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {data.planets?.map((p: any, i: number) => (
              <tr key={p.planet} style={{ borderBottom: '1px solid var(--border)', background: p.shifted ? 'var(--red-bg)' : i % 2 === 0 ? 'var(--surface)' : 'var(--surface2)' }}>
                ...
                <td style={{ padding: '8px 16px' }}>
                  {p.shifted
                    ? <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: 'var(--red-bg)', color: '#DC2626', fontFamily: "'Noto Sans Devanagari', sans-serif" }}>{t('SHIFTED')}</span>
