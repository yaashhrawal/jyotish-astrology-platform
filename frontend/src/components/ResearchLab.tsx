...
const S = {
  ...
  label: { color: 'var(--text3)', fontSize: '12px', marginBottom: '4px', display: 'block' },
  select: { width: '100%', padding: '7px 10px', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '6px', color: 'var(--text)', fontSize: '13px', marginBottom: '10px' },
  btn: { padding: '9px 18px', background: 'var(--accent)', border: 'none', borderRadius: '6px', color: '#fff', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' },
  title: { color: 'var(--accent)', fontSize: '15px', fontWeight: 'bold', marginBottom: '14px' },
  chartCard: { background: 'var(--bg)', border: '1px solid var(--accent-bg)', borderRadius: '8px', padding: '12px', marginBottom: '8px' },
  badge: { fontSize: '11px', padding: '2px 6px', borderRadius: '3px', background: 'var(--accent-bg)', color: 'var(--accent)', margin: '2px' },
  stat: { display: 'inline-block', padding: '8px 14px', background: 'var(--bg)', border: '1px solid var(--accent-bg)', borderRadius: '8px', margin: '4px', textAlign: 'center' as const },
}
    ...
    <div>
      <div style={S.wrap}>
        ...
        <div style={{ ...S.panel, width: '280px', flexShrink: 0 }}>
          ...
          <div style={{ borderTop: '1px solid var(--accent-bg)', paddingTop: '12px', marginTop: '4px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
            ...
            <button style={{ ...S.btn, background: 'var(--accent-bg)' }} onClick={reset}>✕</button>
          </div>
        </div>
          ...
          {stats && !results && (
            <div style={S.panel}>
              ...
              <div style={{ marginBottom: '16px' }}>
                <span style={S.stat}><div style={{ color: 'var(--accent)', fontSize: '22px', fontWeight: 'bold' }}>{stats.total_charts}</div><div style={{ color: 'var(--text3)', fontSize: '11px' }}>Total Charts</div></span>
              </div>
              {stats.ascendant_distribution?.length > 0 && (
                <div>
                  <div style={{ color: 'var(--text3)', fontSize: '12px', marginBottom: '8px' }}>Ascendant Distribution</div>
              ...
              {stats.top_yogas?.length > 0 && (
                <div style={{ marginTop: '14px' }}>
                  <div style={{ color: 'var(--text3)', fontSize: '12px', marginBottom: '8px' }}>Most Common Yogas</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {stats.top_yogas.map((r: any) => (
                      <span key={r.yoga} style={{ ...S.badge, color: 'var(--gold)' }}>{r.yoga}: {r.count}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          {results && (
            <div style={S.panel}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                ...
                <button style={{ ...S.btn, background: 'var(--accent-bg)', fontSize: '12px', padding: '6px 12px' }} onClick={reset}>← Back</button>
              </div>
              {results.charts.length === 0 && (
                <div style={{ color: 'var(--text3)', fontSize: '13px' }}>No charts match these filters.</div>
              )}
              {results.charts.map((c: any) => (
                <div key={c.id} style={S.chartCard}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ color: 'var(--accent)', fontWeight: 'bold', fontSize: '14px' }}>{c.name}</div>
