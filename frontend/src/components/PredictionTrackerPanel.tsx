...
function AccuracyChart({ preds }: { preds: Prediction[] }) {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-m)', padding: '16px' }}>
      <div style={{ fontSize: '13px', fontWeight: '700', marginBottom: '12px' }}>📊 {t('Prediction Accuracy')}</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', marginBottom: '14px' }}>
        {[
          { label: t('Accuracy Score'), val: `${score}%`, color: parseFloat(score) >= 70 ? '#16A34A' : parseFloat(score) >= 50 ? '#F59E0B' : '#DC2626' },
          { label: t('Fulfilled'), val: fulfilled, color: '#16A34A' },
          { label: t('Partial'), val: partial, color: '#0891B2' },
          { label: t('Missed'), val: missed, color: '#DC2626' },
        ].map(({ label, val, color }) => (
      ...
      <div style={{ fontSize: '10px', fontWeight: '700', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '6px' }}>{t('By Category')}</div>
...
export default function PredictionTrackerPanel() {
  ...
  const del = (id: string) => { if (confirm(t('Delete this prediction?'))) save(predictions.filter(p => p.id !== id)) }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      ...
      <div style={{ padding: '14px 18px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-m)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: '15px', fontWeight: '700', marginBottom: '2px' }}>{t('Prediction Tracker')}</div>
          <div style={{ fontSize: '12px', color: 'var(--text3)' }}>{t('Log predictions · track outcomes · measure accuracy — stored locally')}</div>
        </div>
        <div style={{ display: 'flex', gap: '6px' }}>
          {(['list', 'new', 'stats'] as const).map(v => (
            <button key={v} onClick={() => { setView(v); setEditId(null); setForm(BLANK) }} style={{
              ...
              {v === 'list' ? `📋 ${t('All')} (${predictions.length})` : v === 'new' ? `+ ${t('New')}` : `📊 ${t('Stats')}`}
            </button>
          ))}
        </div>
      </div>
      ...
      {(view === 'new' || view === 'edit') && (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-m)', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ fontSize: '13px', fontWeight: '700' }}>{editId ? t('Edit Prediction') : t('New Prediction')}</div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <F label={`${t('Client Name')} *`}>
              <input value={form.client_name} onChange={e => setForm(f => ({ ...f, client_name: e.target.value }))} style={inputStyle} placeholder={t('e.g. Ramesh Patel')} />
            </F>
            <F label={t('Date of Birth')}>
              <input type="date" value={form.dob} onChange={e => setForm(f => ({ ...f, dob: e.target.value }))} style={inputStyle} />
            </F>
          </div>

          <F label={`${t('Prediction')} *`}>
            <textarea value={form.prediction_text} onChange={e => setForm(f => ({ ...f, prediction_text: e.target.value }))}
              rows={3} style={{ ...inputStyle, resize: 'vertical' }} placeholder={t('Describe the prediction in detail…')} />
          </F>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
            ...
            <F label={t('Timeframe')}>
              <input value={form.timeframe} onChange={e => setForm(f => ({ ...f, timeframe: e.target.value }))} style={inputStyle} placeholder={t('e.g. 2025 Q1, within 6 months')} />
            </F>
            <F label={t('Confidence (1–5)')}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <F label={t('Active Dasha / Antardasha')}>
              <input value={form.dasha} onChange={e => setForm(f => ({ ...f, dasha: e.target.value }))} style={inputStyle} placeholder={t('e.g. Saturn-Jupiter')} />
            </F>
            <F label={t('Transit Planets')}>
              <input value={form.transit_planets} onChange={e => setForm(f => ({ ...f, transit_planets: e.target.value }))} style={inputStyle} placeholder={t('e.g. Jupiter 9th, Saturn 10th')} />
            </F>
          </div>
          ...
          <F label={t('Tags')}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              <input value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTag()} style={{ ...inputStyle, flex: 1 }} placeholder={t('Add tag, press Enter')} />
              <button onClick={addTag} style={{ padding: '9px 14px', borderRadius: 'var(--radius-m)', border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', cursor: 'pointer', fontSize: '12px' }}>{t('Add')}</button>
            </div>
          </F>
          ...
          {editId && (
            <>
              <div style={{ borderTop: '1px solid var(--border)', paddingTop: '12px' }}>
                <div style={{ fontSize: '11px', fontWeight: '700', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '10px' }}>{t('Outcome (update when result known)')}</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <F label={t('Status')}>
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
            <button onClick={() => { setView('list'); setForm(BLANK); setEditId(null) }} style={{ padding: '9px 18px', borderRadius: 'var(--radius-m)', border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text3)', cursor: 'pointer', fontSize: '13px' }}>{t('Cancel')}</button>
            <button onClick={submit} style={{ padding: '9px 24px', borderRadius: 'var(--radius-m)', border: 'none', background: 'var(--accent)', color: '#fff', fontWeight: '700', cursor: 'pointer', fontSize: '13px' }}>
              {editId ? t('Update') : t('Save Prediction')}
            </button>
          </div>
        </div>
      )}
      ...
      {view === 'list' && (
        <>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <input value={filterClient} onChange={e => setFilterClient(e.target.value)} style={{ flex: 1, minWidth: '120px', padding: '8px 10px', borderRadius: 'var(--radius-m)', border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)', fontSize: '12px' }} placeholder={t('Filter by client…')} />
          ...
          {visible.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text3)', fontSize: '13px' }}>
              {predictions.length === 0 ? t('No predictions yet. Click "+ New" to add your first prediction.') : t('No predictions match the filter.')}
            </div>
          )}
                ...
                <div key={p.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderLeft: `4px solid ${sc}`, borderRadius: 'var(--radius-m)', padding: '12px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      {p.outcome_notes && (
                        <div style={{ marginTop: '8px', padding: '8px 10px', background: sc + '10', borderRadius: '6px', fontSize: '11.5px', color: 'var(--text2)', fontStyle: 'italic' }}>
                          <span style={{ fontStyle: 'normal', fontWeight: '700', color: sc }}>{t('Outcome')}: </span>{p.outcome_notes}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', flexShrink: 0 }}>
                      <button onClick={() => cycleStatus(p)} title={t('Cycle status')} style={{ padding: '5px 10px', borderRadius: '6px', border: `1px solid ${sc}`, background: sc + '18', color: sc, fontSize: '11px', cursor: 'pointer', fontWeight: '700' }}>
