...
export default function ConditionalDashaPanel({ birthData }: { birthData: BirthData | null }) {
  ...
  if (!birthData) return <div style={{ padding: 20, color: 'var(--text3)', fontSize: 13 }}>{t('Enter birth data to compute conditional dashas.')}</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#a78bfa' }}>{t('Conditional Dashas')}</div>
          <div style={{ fontSize: 11, color: 'var(--text3)' }}>{t('Dwisaptati · Shodashottari · Shatabdika · Dwadasottari')}</div>
        </div>
      ...
      {data && (
        <>
            ...
            {SYSTEM_KEYS.map(key => {
              ...
              return (
                <div key={key} onClick={() => setActiveSystem(key)} style={{
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text3)' }}>{sys.total_years} {t('years')}</div>
                  <div style={{ fontSize: 11, marginTop: 4, fontWeight: 600, color: sys.condition_met ? '#22c55e' : 'var(--text4)' }}>
                    {sys.condition_met ? `✓ ${t('Active')}` : `✗ ${t('Inactive')}`}
                  </div>
                </div>
              )
            })}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 11, padding: '2px 10px', borderRadius: 10, fontWeight: 600, background: active.condition_met ? '#16A34A20' : '#DC262620', color: active.condition_met ? '#16A34A' : '#DC2626' }}>
                    {active.condition_met ? `✓ ${t('Applicable')}` : `✗ ${t('Not Applicable')}`}
                  </span>
                </div>
                <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 4 }}>{t('Condition')}: {active.condition}</div>
                ...
                {!active.condition_met && (
                  <div style={{ marginTop: 10, padding: '8px 12px', background: 'var(--surface2)', borderRadius: 6, fontSize: 12, color: 'var(--text3)', fontStyle: 'italic' }}>
                    {t('This dasha system does not apply for this chart. Sequence shown for reference only.')}
                  </div>
                )}
              </div>
              ...
              <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
                  <span style={{ fontSize: 13, fontWeight: 600 }}>{t('Dasha Sequence')}</span>
                  <span style={{ fontSize: 11, color: 'var(--text3)' }}>{active.total_years}-{t('year cycle')}</span>
                </div>
                    ...
                    <thead>
                      <tr style={{ background: 'var(--surface2)' }}>
                        {['#', 'Lord / Sign', 'Years', 'Start', 'End'].map(h => (
                          <th key={h} style={{ padding: '8px 14px', textAlign: 'left', fontSize: 11, color: 'var(--text3)', fontWeight: 600, borderBottom: '1px solid var(--border)' }}>{t(h)}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                ...
                {active.sequence?.length > 12 && (
                  <div style={{ padding: '8px 16px', borderTop: '1px solid var(--border)' }}>
                    <button onClick={() => setShowAll(!showAll)} style={{ fontSize: 12, color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer' }}>
                      {showAll ? t('Show less') : `${t('Show all')} ${active.sequence.length} ${t('periods')}`}
                    </button>
                  </div>
                )}
              </div>
          ...
          <div style={{ ...card, fontSize: 12, color: 'var(--text3)', lineHeight: 1.7 }}>
            <div style={{ fontWeight: 600, color: 'var(--text2)', marginBottom: 6 }}>{t('About Conditional Dashas')}</div>
            <div><span style={{ color: '#6366f1' }}>{t('Dwisaptati Sama (72yr)')}</span> — {t('8 lords × 9yr. Active when lagna lord in 7H or 7th lord in lagna.')}</div>
            <div><span style={{ color: '#a78bfa' }}>{t('Shodashottari (116yr)')}</span> — {t('9 lords. Active when Moon occupies Hora Lagna. Rare, precise timing.')}</div>
            <div><span style={{ color: '#f59e0b' }}>{t('Shatabdika (100yr)')}</span> — {t('Sign-based, 8.33yr/sign from lagna. General life span mapping.')}</div>
            <div><span style={{ color: '#10b981' }}>{t('Dwadasottari (112yr)')}</span> — {t('9 lords. Active when Venus in lagna or 7H. Excellent for relationships.')}</div>
          </div>
        </>
      )}
    </div>
