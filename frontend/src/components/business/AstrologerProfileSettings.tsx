    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      <div style={{ width: 70, height: 70, border: '1px dashed var(--border)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface2)', overflow: 'hidden' }}>
        {currentUrl ? <img src={currentUrl} alt={kind} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                     : <span style={{ fontSize: 10, color: 'var(--text4)' }}>No {kind}</span>}
      </div>
...
export default function AstrologerProfileSettings() {
  ...
  const save = async () => {
    setSaving(true)
    try { await profileApi.save(p); toast.success('Profile saved') }
    catch (e: any) { toast.error(e?.response?.data?.detail || 'Save failed') }
    finally { setSaving(false) }
  }

  if (loading) return <div style={{ padding: 20, color: 'var(--text3)' }}>Loading profile…</div>

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 900 }}>
      <div>
        <div style={{ fontSize: 16, fontWeight: 700 }}>Brand & PDF Letterhead</div>
        <div style={{ fontSize: 12, color: 'var(--text3)' }}>Used on every PDF report and receipt you generate.</div>
      </div>

      <div style={card}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12 }}>Brand Assets</div>
      ...
      <div style={card}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12 }}>Identity</div>
        <div style={grid2}>
          <Field name="Display Name" value={p.display_name} onChange={set('display_name')} placeholder="Pt. Rakesh Sharma" />
          <Field name="Title" value={p.title} onChange={set('title')} placeholder="Jyotish Acharya · MA Vedic Astrology" />
          <Field name="Registration No." value={p.registration_no} onChange={set('registration_no')} placeholder="KSAB-2018-1234" />
          <Field name="Tagline" value={p.tagline} onChange={set('tagline')} placeholder="30 years of accurate predictions" />
        </div>
        <div style={{ marginTop: 12 }}>
          <div style={label}>Qualifications (comma separated)</div>
      ...
      <div style={card}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12 }}>Contact</div>
        <div style={grid2}>
          <Field name="Phone" value={p.phone} onChange={set('phone')} />
          <Field name="WhatsApp" value={p.whatsapp} onChange={set('whatsapp')} />
          <Field name="Email" value={p.email} onChange={set('email')} type="email" />
          <Field name="Website" value={p.website} onChange={set('website')} placeholder="https://" />
        </div>
      </div>

      <div style={card}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12 }}>Address</div>
        <div style={grid2}>
          <Field name="Address Line 1" value={p.address_line1} onChange={set('address_line1')} />
          <Field name="Address Line 2" value={p.address_line2} onChange={set('address_line2')} />
          <Field name="City" value={p.city} onChange={set('city')} />
          <Field name="State" value={p.state} onChange={set('state')} />
          <Field name="Pincode" value={p.pincode} onChange={set('pincode')} />
        </div>
      </div>

      <div style={card}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 12 }}>Tax & Compliance</div>
        <div style={grid2}>
          <Field name="GSTIN" value={p.gst_number} onChange={set('gst_number')} placeholder="27AAACR1234F1Z5" />
          <Field name="PAN" value={p.pan_number} onChange={set('pan_number')} placeholder="AAACR1234F" />
        </div>
      </div>
      ...
      <div style={card}>
        ...
        <div style={grid2}>
          <div>
            <div style={label}>Primary Color</div>
          ...
          <div>
            <div style={label}>Secondary Color</div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <div style={{ marginTop: 12 }}>
          <div style={label}>PDF Footer Quote (Sanskrit shloka or motto)</div>
        ...
        <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12 }}>Show "Powered by JyotishApp" footer (Pro plan can hide)</span>
        </div>
      </div>
