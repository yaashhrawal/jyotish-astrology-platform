...
export default function VarnadaPanel({ birthData }: { birthData: BirthData | null }) {
  ...
  if (!birthData) return <div style={{ padding: 20, color: 'var(--text3)', fontSize: 13 }}>{t('Enter birth data to compute Varnada Lagna.')}</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: '#10b981' }}>{t('Varnada Lagna')}</div>
          <div style={{ fontSize: 11, color: 'var(--text3)' }}>{t('Jaimini Longevity & Life Timing')}</div>
        </div>
        ...
        <>
          ...
          <div style={card}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 16 }}>
              {[
                { label: t('Varnada Lagna'), val: data.varnada_lagna?.sign, sub: ORDINALS[data.varnada_lagna?.house_in_d1] + ' ' + t('house'), color: '#10b981' },
                { label: t('VL Lord'), val: data.varnada_lagna?.lord, sub: `${t('in')} ${data.vl_lord_analysis?.sign} (${ORDINALS[data.vl_lord_analysis?.house]}H)`, color: PLANET_COLORS[data.varnada_lagna?.lord] || 'var(--accent)' },
                { label: t('Birth Type'), val: data.computation?.day_birth ? `☀ ${t('Day')}` : `🌙 ${t('Night')}`, sub: `${data.computation?.hours_from_sunrise?.toFixed(1)}h ${t('from sunrise')}`, color: 'var(--text)' },
                { label: t('Hora Lagna'), val: data.hora_lagna?.sign, sub: '', color: 'var(--text2)' },
            ...
            <div style={{ paddingTop: 12, borderTop: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#10b981' }}>{t('Longevity Assessment')}</span>
                <span style={{ fontSize: 13, fontWeight: 700 }}>{data.longevity?.longevity}</span>
              </div>
              <ScoreBar score={data.longevity?.score || 0} />
              <div style={{ fontSize: 11, color: 'var(--text4)', marginTop: 4 }}>{t('Estimated span')}: {data.longevity?.span}</div>
            </div>
          </div>

          {/* Computation */}
          <div style={card}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#10b981', marginBottom: 12 }}>{t('Computation (Jaimini Sutras 1.1.28–31)')}</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                [data.computation?.day_birth ? t('Count Aries→Lagna') : t('Count Lagna→Aries'), data.computation?.count_lagna],
                [data.computation?.day_birth ? t('Count Aries→Hora Lagna') : t('Count Hora Lagna→Aries'), data.computation?.count_hl],
              ].map(([label, val]) => (
              ...
              <div style={{ padding: '10px 14px', background: 'var(--surface2)', borderRadius: 8, gridColumn: '1 / -1' }}>
                <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 4 }}>
                  {t('Sum')} = {data.computation?.total} → {t('Varnada Lagna')}
                </div>
          ...
          {data.vl_theme?.theme && (
            <div style={card}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#10b981', marginBottom: 8 }}>{t('Life Theme — VL in')} {data.varnada_lagna?.sign}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={card}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#10b981', marginBottom: 10 }}>{t('VL Lord')} — {data.vl_lord_analysis?.planet}</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 10 }}>
                {[['Sign', data.vl_lord_analysis?.sign], ['House', ORDINALS[data.vl_lord_analysis?.house]], ['Dignity', (data.vl_lord_analysis?.status || '').replace('_', ' ')]].map(([l, v]) => (
                  <div key={l as string}>
                    <div style={{ fontSize: 11, color: 'var(--text3)' }}>{t(l as string)}</div>
            ...
            <div style={card}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#f97316', marginBottom: 6 }}>{t('8th from VL')} — {data.eighth_from_vl?.sign}</div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 8 }}>{t('Maraka house from VL')}</div>
              ...
              ) : <span style={{ fontSize: 12, color: 'var(--text4)' }}>{t('No planets — favorable for longevity')}</span>}
              {data.eighth_from_vl?.planets?.some((p: string) => ['Saturn','Mars','Rahu','Ketu'].includes(p)) && (
                <div style={{ fontSize: 11, color: '#ef4444', marginTop: 8 }}>⚠ {t('Malefics in 8th from VL — longevity factor reduced')}</div>
              )}
            </div>
          </div>
          ...
          {data.planets_in_vl?.length > 0 && (
            <div style={{ ...card, display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
              <span style={{ fontSize: 12, color: 'var(--text3)' }}>{t('Planets in VL')}:</span>
          ...
          <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
            <button onClick={() => setShowPeriods(!showPeriods)} style={{
              ...
              <span>{t('Varnada Periods (Life Stage Timing)')}</span>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: 'var(--surface2)' }}>
                      {['#', t('Sign'), t('Lord'), t('Age'), t('Theme')].map(h => (
          ...
          <div style={{ ...card, fontSize: 12, color: 'var(--text3)', lineHeight: 1.6 }}>
            <strong style={{ color: 'var(--text2)' }}>{t('Source')}:</strong> {t('Jaimini Sutras 1.1.28–31. Varnada Lagna differs for day/night births by counting from Aries to Lagna and Hora Lagna, then summing. Used for longevity and life-stage timing.')}
          </div>
        </>
      )}
    </div>
