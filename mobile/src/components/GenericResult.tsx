...
const fmt = (v: any): string => {
  ...
  if (isObj(v)) return v.name || v.label || v.sign || '—';
  return String(v);
};

function Table({ rows, c, s }: { rows: any[]; c: Colors; s: any }) {
  const cols = Array.from(new Set(rows.flatMap((r) => Object.keys(r)))).filter((k) => !SKIP.has(k)).slice(0, 5);
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
      <View>
        <View style={[s.tr, s.trh]}>
          {cols.map((col) => <Text key={col} style={[type.micro, s.th]}>{title(col).slice(0, 12)}</Text>)}
        </View>
        {rows.slice(0, 40).map((r, i) => (
          <View key={i} style={s.tr}>
            {cols.map((col) => <Text key={col} style={[type.caption, s.td]} numberOfLines={2}>{fmt(r[col])}</Text>)}
          </View>
        ))}
      </View>
    </ScrollView>
...
function Node({ value, depth, c, s }: { value: any; depth: number; c: Colors; s: any }) {
  if (isPrim(value)) return <Text style={[type.body, { color: c.textSecondary }]}>{fmt(value)}</Text>;

  if (Array.isArray(value)) {
    if (!value.length) return <Text style={[type.body, { color: c.textMuted }]}>None</Text>;
    if (value.every(isPrim)) return <Text style={[type.body, { color: c.textSecondary }]}>{value.map(fmt).join(' · ')}</Text>;
    return <Table rows={value.filter(isObj)} c={c} s={s} />;
  }
    ...
    <View>
        ...
        <View style={{ marginBottom: complex.length ? spacing.sm : 0 }}>
          {prims.map(([k, v]) => (
            <View key={k} style={s.kv}>
              <Text style={[type.caption, { color: c.textMuted, width: 130 }]}>{title(k)}</Text>
              <Text style={[type.body, { color: c.textPrimary, flex: 1 }]}>{fmt(v)}</Text>
            </View>
          ))}
        </View>
      )}
      {complex.map(([k, v]) => (
        <View key={k} style={{ marginTop: spacing.sm }}>
          <Text style={[type.micro, { color: c.accentPrimary, marginBottom: 4 }]}>{title(k).toUpperCase()}</Text>
          {depth < 3 ? <Node value={v} depth={depth + 1} c={c} s={s} /> : <Text style={[type.caption, { color: c.textMuted }]}>{fmt(v)}</Text>}
        </View>
      ))}
    </View>
  );
...
export default function GenericResult({ data }: { data: any }) {
  ...
  if (!data) return null;
  return <Node value={data} depth={0} c={c} s={s} />;
}
